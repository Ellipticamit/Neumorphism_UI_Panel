  ...
  const mapData = (value) => {
    if (gridView) {
      return value.map((item) => (
        <Card data={item} key={item.country} theme={theme} />
      ));
    } else {
      return value.map((item) => (
        <List data={item} key={item.country} theme={theme} />
      ));
    }
  };
      ...
      <div className='container'>
        <div className='top-header'>
          ...
          <div className='sort-section'>
            <SelectBox sortvalue={getSortValue} />
          </div>
        </div>
        <Search search={searchData} view={handleView} />
        <div className='card-container'>{getData()}</div>
      </div>
    </div>
  );
};
